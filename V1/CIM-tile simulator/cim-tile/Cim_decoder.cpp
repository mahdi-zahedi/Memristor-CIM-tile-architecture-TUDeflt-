...
{	
	...
	CS_index = new char[(int)log2(Number_of_Cols / Number_of_ADCs) + 1];
	...
	while (getline(infile1, line) && !infile1.eof())
	{
		...
		switch (s1_opcode1[s1_string_opcode1])
		{
			...
			{				
				if (inst_bypass_flag)
					break;
				char* RDSri_index = new char[(int)ceil(log2((RDS_bandwidth/log2(Number_of_Rows))))+1];
			...
			{	
				...
				if (inst_bypass_flag)
					break;
				char* WD_index = new char[(int)log2(Number_of_Cols / WD_buffer_bandwidth) + 1];
			...
			case WDSb:
			{
				...
				if (inst_bypass_flag)
					break;
				char* WDS_index = new char[(int)log2(Number_of_Cols/WDS_bandwidth)+1];
