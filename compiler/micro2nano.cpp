...
const int num_of_ADC = 16; const char WDS_for_all_operations = 'n'; int Max_row_select = 256; const int datatype_size = 16; const int max_datatype_size = 16;
...
void read_memfile(int*, ifstream&);
...
int main()
{
	...
	ofstream nanofile; // All nano instructions generated by the compiler
	ofstream WDfile; // Write data file to set the memristor values
	ofstream RDfile; // Row data file to pass to the xbar inputs
	...
	// Load data for WD buffer
	ifstream wd_mem_file;
	wd_mem_file.open("wd_mem_input.txt");
	read_memfile(intMem1, wd_mem_file);
	dispMem(intMem1, memfile1); // display WD mem

	// Load data for RD buffer
	ifstream rd_mem_file;
	rd_mem_file.open("rd_mem_input.txt");
	read_memfile(intMem2, rd_mem_file);
	dispMem(intMem2, memfile2); // display RD mem
	...
	while (getline(infile, line))
	{
		...
		case store:
		{
			...
			string temp_string;
			iss >> op1_address;
			cout << "op1 addr: " << op1_address << endl;
			for (i = 3; i < op1_address.size(); i++)
				...
				else
				{
					...
					cout << "temp int: " << temp_string << endl;
				}
			cout << "temp end:" << temp_string << endl;
			...
			for (i = i + 2; i < op1_address.size(); i++)
				...
				{
					temp_string.push_back(op1_address.at(i));
					cout << "temp2 int: " << temp_string << endl;
				}
			cout << "temp2 end: " << temp_string << endl;
...
/// @brief Puts the data of a memory object (which represents the main memory) to a file
/// @param memory Pointer to the memory object which represents the main memory
/// @param dispMem Reference to ofstream object of the file
...
/// @brief Initializes the main memory with random data
/// @param memory Memory object which represents the main memory
void intMem_init(int* memory) {
	srand(time(NULL));
	for (int i = 0; i < mem1_row_size; i++) {
		for (int k = 0; k < mem1_column_size; k++) {
			*(memory + k + i * mem1_column_size) = 0;
			// *(memory + k + i * mem1_column_size) = rand() % memristor_level;
		}
	}
}

/// @brief Initializes the memory with data from a given file
/// @param rd_memory Memory object which represents the memory
/// @param rd_memfile Reference to ifstream object of the data file 
void read_memfile(int* memory, ifstream& memfile){
	int data; // Variable buffer for storing current data to process
	std::bitset<16> data_bitset; // Variable buffer for storing rd_data in bit representation
	for (int i = 0; i < datatype_size; i++){
		for (int j = 0; j < datatype_size; j++){
			memfile >> data; // Put current element into data buffer
			data_bitset = std::bitset<16>(data); // Convert current (integer element) to bits
			for (int k = 0; k < datatype_size; k++){
				*(memory + i*mem1_row_size + j*datatype_size + k) = data_bitset[datatype_size - k - 1]; // Put every bit in the memory object
			}
		}
	}
}
...
void Initialize()
{
	...
	opcode["read"] = read;
	opcode["MMM"] = MMM;
	opcode["logical_and"] = logical_and1;
	opcode["logical_or"] = logical_or1;
